...
import { matchSelfie } from '../utils/mockFaceMatch'
...
export default function Results() {
  ...
  // Download all matched images as a zip using their Cloudinary URLs
    ...
    await Promise.all(
      matchedPhotos.map(async (photo) => {
        try {
          ...
          console.error("Failed to fetch image for zip:", photo.image_name, err)
        }
      })
    )
  ...
  useEffect(() => {
    if (!selfie || !albumUploaded || !albumId) {
      navigate('/')
      return
    }
    ...
    if (matchedPhotos.length === 0) {
      const runMatch = async () => {
        try {
          ...
          // Send selfie + album_id; receive [{ album_id, image_name, image_url, score }]
          const matches = await matchSelfie(selfie, albumId)
      ...
      <div className="w-full max-w-4xl">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-32 gap-8 animate-fade-in">
            ...
            <div className="text-center">
              ...
              <p className="text-ink-600 text-sm">Matching your selfie against the album</p>
            </div>
        ...
        {!loading && matchedPhotos.length > 0 && (
          <>
            ...
            <div className="flex items-center gap-4 mb-8 p-4 rounded-2xl bg-ink-800/60 border border-ink-700 animate-fade-up">
              ...
              <div>
                <p className="font-display font-600 text-sm text-white" style={{ fontWeight: 600 }}>
                  Matched against your event album
                </p>
            ...
            {/* Photo grid — images served directly from Cloudinary */}
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mb-10 animate-fade-up">
              {matchedPhotos.map((photo, i) => (
                <div
                  ...
                  onClick={() => setLightbox({ url: photo.image_url, name: photo.image_name })}
                >
                  <img
                    src={photo.image_url}
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-3">
                    <div className="flex items-center justify-between w-full">
                      <p className="text-white text-xs truncate max-w-[70%]">{photo.image_name}</p>
                      <button
                        onClick={(e) => { e.stopPropagation(); handleDownload(photo.image_url, photo.image_name) }}
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <button onClick={handleDownloadAll} className="btn-primary text-sm py-3 px-6">
                Download All
              </button>
              <button
        ...
        {!loading && albumUploaded && matchedPhotos.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 animate-fade-in text-center">
            ...
            <h2 className="font-display text-3xl font-700 mb-3">No Photos Found</h2>
            ...
            <button onClick={handleRestart} className="btn-primary text-sm py-3 px-6">
              Try Again
            </button>
          </div>
        )}
