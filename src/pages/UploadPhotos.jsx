...
export default function UploadPhotos() {
  ...
  const { eventPhotos, addEventPhotos, removeEventPhoto, setAlbumUploaded, setAlbumId } = usePhoto()
  ...
  const handleFiles = (files) => {
    ...
    // Enforce 200-image limit before adding
    const totalAfterAdd = eventPhotos.length + files.length
    if (totalAfterAdd > MAX_IMAGES) {
      setUploadError(`You can upload a maximum of ${MAX_IMAGES} images per album.`)
      return
    }

    addEventPhotos(files)
  }

  const handleNavigation = async () => {
    if (eventPhotos.length > MAX_IMAGES) {
      setUploadError(`You can upload a maximum of ${MAX_IMAGES} images per album.`)
      return
    }
    ...
    try {
      ...
      // Backend generates all IDs — we only send files
      const data = await uploadAlbum(eventPhotos)

      // Store the backend-generated album_id for use in /match_selfie
      ...
      <div className="w-full max-w-3xl">
          ...
          <div className="flex flex-col items-center gap-4 pointer-events-none">
            ...
            <div>
              ...
              <p className="text-amber-300 text-xs mt-1">
                Maximum {MAX_IMAGES} images per album
              </p>
            </div>
          </div>
        </DropZone>
        ...
        {eventPhotos.length > 0 && (
          <p className="text-center text-sm text-amber-400 mb-4 animate-fade-in">
            {eventPhotos.length} / {MAX_IMAGES} photo{eventPhotos.length !== 1 ? 's' : ''} loaded
          </p>
        )}
        ...
        <div className="flex justify-center">
          <button
            disabled={eventPhotos.length === 0 || loading}
